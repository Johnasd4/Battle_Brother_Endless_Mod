	...
	if (_element !== null && _element.length > 0)
	{
		...
		if (_element.hasClass('is-selected') !== true)
		{
			...
			this.notifyBackendTentSelected(id, function(res) {
				...
				self.mTentButton.changeButtonText(TranslateButton(res.Label));
...
CampScreenCommanderDialogModule.prototype.addListEntry = function (_data)
{
    ...
    var name = $('<div class="name title-font-normal font-bold font-color-brother-name">' + TranslateWorldScreenCampDialogModule(_data[WorldTownScreenIdentifier.HireRosterEntry.Name]) + '</div>');
	row.append(name);
