...
WorldCombatDialog.prototype.loadFromData = function (_data)
{
    ...
    this.mDialogContainer = dialogLayout.createDialog(allowDisengage ? '准备战斗' : '你被攻击了', null, /*Path.GFX + Asset.HEADER_TACTICAL_COMBAT_DIALOG*/null, true, 'dialog-800-720-2');
	...
	{
		var titleLabel = $('<div class="title-empty-label text-font-medium font-color-subtitle">' + TranslateDialog(_data.Text) + '</div>');
		rightColumn.append(titleLabel);
	}
	else
	{
		var titleLabel = $('<div class="title-label text-font-medium font-color-subtitle">你的侦察员报告说他看到了以下情况…</div>');
		rightColumn.append(titleLabel);
		...
		for(var i = 0; i < entities.length; ++i)
		{
            ...
            table += '</div></td > <td width="5%"/> <td width="85%" class="entity-label text-font-medium font-color-description">' + TranslateToolTip0(entities[i]['Name']) + '</td></tr >';
		}
