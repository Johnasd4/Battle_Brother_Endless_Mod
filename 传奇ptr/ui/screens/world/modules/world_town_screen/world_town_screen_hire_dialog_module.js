...
WorldTownScreenHireDialogModule.prototype.createDIV = function (_parentDiv)
{
    ...
    var costsLabel = $('<div class="costs-label title-font-normal font-bold font-bottom-shadow font-color-title">支付</div>');
    ...
    costsLabel = $('<div class="costs-label title-font-normal font-bold font-bottom-shadow font-color-title">每日</div>');
    ...
    this.mDetailsPanel.HireButton = hireButtonLayout.createTextButton("雇佣", function()
	{
		if(self.mSelectedEntry !== null)
		{
			var data = self.mSelectedEntry.data('entry');
    ...
    this.mDetailsPanel.TryoutButton = tryoutButtonLayout.createTextButton("试用", function()
	{
		if(self.mSelectedEntry !== null)
		{
			var data = self.mSelectedEntry.data('entry');
	...
	this.mDetailsPanel.DismissButton = this.dismissButtonLayout.createTextButton("解雇", function()
	{
		if(self.mSelectedEntry !== null)
		{
			var data = self.mSelectedEntry.data('entry');
	...
	this.mLeaveButton = layout.createTextButton("离开", function ()
	{
...
WorldTownScreenHireDialogModule.prototype.addListEntry = function (_data)
{
    ...
    var name = $('<div class="name title-font-normal font-bold font-color-title">' +TranslateToolTip0(TranslateCharacterNames(_data[WorldTownScreenIdentifier.HireRosterEntry.Name])) + '</div>');
	row.append(name);
...
WorldTownScreenHireDialogModule.prototype.updateDetailsPanel = function(_element)
{
	if (_element !== null && _element.length > 0)
	{
        ...
        this.mDetailsPanel.CharacterName.html(TranslateToolTip0(TranslateCharacterNames(data['Name'])));
