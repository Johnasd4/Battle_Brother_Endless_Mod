...
CharacterScreenLeftPanelHeaderModule.prototype.createChangeNameAndTitleDialogContent = function (_dialog)
{
	...
	result.append(row);
    var label = $('<div class="label text-font-normal font-color-label font-bottom-shadow">名称</div>');
	row.append(label);
	...
	row = $('<div class="row"/>');
	result.append(row);
    label = $('<div class="label text-font-normal font-color-label font-bottom-shadow">头衔</div>');
	row.append(label);
...
CharacterScreenLeftPanelHeaderModule.prototype.createDismissDialogContent = function (_dialog)
{
	...
	if (selectedBrother['dailyMoneyCost'] == 0)
        titleLabel = $('<div class="title-label title-font-normal font-bold font-color-title">真的释放 ' + selectedBrother[CharacterScreenIdentifier.Entity.Character.Name] + '?</div>');
	else
        titleLabel = $('<div class="title-label title-font-normal font-bold font-color-title">真的解雇 ' + selectedBrother[CharacterScreenIdentifier.Entity.Character.Name] + '?</div>');

	result.append(titleLabel);

	var textLabel = $('<div class="label text-font-medium font-color-description font-style-normal">' + selectedBrother[CharacterScreenIdentifier.Entity.Character.Name] + ' 会永远离开你，并将他 <br/>当前的装备放到仓库中。</div>');
	...
	if (selectedBrother['dailyMoneyCost'] == 0)
        checkboxLabel = $('<label class="blub text-font-medium font-color-subtitle font-style-normal" for="compensation">支付 <img src="' + Path.GFX + Asset.ICON_MONEY_SMALL + '"/>' + (Math.max(1, selectedBrother['daysWithCompany']) * 10) + ' 赔偿</label>');
	else
        checkboxLabel = $('<label class="blub text-font-medium font-color-subtitle font-style-normal" for="compensation">支付 <img src="' + Path.GFX + Asset.ICON_MONEY_SMALL + '"/>' + (Math.max(1, selectedBrother['daysWithCompany']) * 10) + ' 补偿</label>');
...
CharacterScreenLeftPanelHeaderModule.prototype.setXP = function(_xpValue, _xpValueMax, _level, _hasLevelUp)
{
	...
	if (_hasLevelUp)
	{
        ...
        this.mXPProgressbar.changeProgressbarLabel(TranslateCharacterScreenLeftPanelHeaderModule('Click here to level up!'));
	}
	else
	{
