...
OptionsMenuModuleAudioPanel.prototype.createDIV = function (_parentDiv)
{
	...
	var displayTile = $('<div class="title title-font-big font-bold font-color-title">音量</div>');
	row.append(displayTile);
	
	this.createVolumeControlDIV(this.mVolumeOptions.Master, '整体', row);	
	this.createVolumeControlDIV(this.mVolumeOptions.Music, '音乐', row);
	this.createVolumeControlDIV(this.mVolumeOptions.Effects, '音效', row);
    this.createVolumeControlDIV(this.mVolumeOptions.Ambience, '环境', row);

	var row = $('<div class="row"></div>');
	rightColumn.append(row);
    var displayTile = $('<div class="title title-font-big font-bold font-color-title">高级</div>');
	row.append(displayTile);
    ...
    this.mHWLabel = $('<label class="text-font-normal font-color-subtitle" for="cb-hw">硬件声音</label>');
