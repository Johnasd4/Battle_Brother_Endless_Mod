::Const.LegendMod.addSettings <- function ()
{
	...
	::Legends.Mod.ModSettings.getSetting("LandRatio").unlock();
	...
	::Legends.Mod.ModSettings.getSetting("Water").unlock();
	...
	::Legends.Mod.ModSettings.getSetting("Snowline").unlock();
	addNCSetting(map, ::MSU.Class.RangeSetting("Settlements", 19, 19, 40, 1, "Settlements", "Maximum number of settlements. Depending on map size, this will try to add the number of settlements on the slider. It will keep the same ratio of settlement types as default Battle Brothers maps. Minimum distance between settlements is 12 tiles. Vanilla default is 19."));
	::Legends.Mod.ModSettings.getSetting("Settlements").unlock();
	...
	::Legends.Mod.ModSettings.getSetting("Factions").unlock();
...
//	addNCSetting(config, ::MSU.Class.BooleanSetting("CraftingUnlocked", true, "All Crafting Recipes Unlocked", "If enabled, all crafting recipes will be visible at the taxidermist and camp crafting. If disabled, vanilla behavior of having to discover the ingredients first will be enforced."));
	...
	::Legends.Mod.ModSettings.getSetting("UnlayeredArmor").lock("Locked for stability.");
	local combat = ::Legends.Mod.ModSettings.addPage("Combat");
	combat.addElement(::MSU.Class.BooleanSetting("EnhancedTooltips", true, "Enhanced Enemy Tooltips", "Enemy tooltips in tactical battles will show more information, like perks and statuses"));
