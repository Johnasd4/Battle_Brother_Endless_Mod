this.el_condition_mult_entry <- this.inherit("scripts/skills/el_entrys/el_entry", {
	...
	function EL_getEntryColour()
	{
        ...
        {
            if (this.m.EL_ConditionMultAddition <= this.Const.EL_Armor.EL_Entry.Factor.EL_ConditionMult.ColourRange[index])
            {
	...
	function EL_createAddition()
	{
		...
		this.m.EL_ConditionMultAddition = this.Const.EL_Armor.EL_Entry.Factor.EL_ConditionMult.BaseConditionMult + this.Math.rand(randomMin, randomMax) * 0.01;
	}
	...
	{
		_EL_totalEntry.m.EL_ArmorConditionMultAddition += this.m.EL_ConditionMultAddition;
	}
