this.town_shop_dialog_module <- this.inherit("scripts/ui/screens/ui_module", {
	...
	function EL_onUpgradeItem( _itemIndex )
	{
		local item = this.Stash.getItemAtIndex(_itemIndex).item;
		...
		local essence_number = item.EL_getUpgradeEssence();
		this.m.WorldTroop.Party.EL_addEquipmentEssence(item.EL_getRankLevel(), essence_number);
	...
	function EL_onRecraftItem( _itemIndex )
	{
		local item = this.Stash.getItemAtIndex(_itemIndex).item;
		if (!this.m.Shop.isRepairOffered() && item != null)
		{
	...
	function EL_onDisassembleItem( _itemIndex )
	{
		local item = this.Stash.getItemAtIndex(_itemIndex).item;
		if (!this.m.Shop.isRepairOffered() && item != null)
		{
		...
		local essence_number = item.EL_getDisassembleEssence();
		this.m.WorldTroop.Party.EL_addEquipmentEssence(item.EL_getRankLevel(), essence_number);
	...
	function onSwapItem( _data )
	{
		...
		case "world-town-screen-shop-dialog-module.stash":
			...
			else
			{
				...
				if (removedItem != null)
				{
					//this.logInfo("information：出售了一件装备————————————————————————————————————————————————————————————————————————————————————————————————————");
