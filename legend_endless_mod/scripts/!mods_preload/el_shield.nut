...
::mods_queue(null, "el_item", function ()
{
    ::mods_hookExactClass("items/shields/shield", function ( o )
	{
		...
		o.applyShieldDamage = function ( _damage, _playHitSound = true )
		{
			...
			if(container != null)
			{
				skill = container.getActor().getSkills().getSkillByID("el_rarity_entry.faith_of_the_rock");
			}
		...
		o.EL_getDisassembleEquipmentEssenceNum <- function()
		{
			...
			{
				result[this.m.EL_RankLevel] += this.Math.ceil(this.Const.EL_Shield.EL_EquipmentEssence.DisassembleFactor * this.Math.abs(this.Math.min(min_calculate_weight, this.m.EL_BaseNoRankStaminaModifier)));
			}
