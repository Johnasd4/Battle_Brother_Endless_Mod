	...
	function getTooltip( _id )
	{
		...
		if(this.m.EL_CurrentLevel != 1)
		{
			return {
				id = _id,
				type = "text",
				text = "[color=" + colour + "]被南方强盗攻击时额外 + " + this.Math.ceil(this.m.EL_CurrentLevel * this.m.EL_Bonus) + "身体血量固定减伤和身体护甲固定减伤[/color]"
			};
		}
		else
		{
			return {
				id = _id,
				type = "text",
				text = "[color=" + colour + "]被南方强盗攻击时额外 + " + this.Math.ceil(this.m.EL_Bonus) + "身体血量固定减伤和身体护甲固定减伤[/color]"
			};
		}
	}	
