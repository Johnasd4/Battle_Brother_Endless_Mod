this.el_value_mult_entry <- this.inherit("scripts/skills/el_entrys/el_entry", {
	...
	function EL_getEntryColour()
	{
        ...
        {
            if (this.m.EL_ValueMultAddition <= this.Const.EL_Shield.EL_Entry.Factor.EL_ValueMult.ColourRange[index])
            {
	...
	{
		local randomMin = this.Const.EL_Shield.EL_Entry.Factor.EL_ValueMult.RandomMinValueMult[this.getItem().m.EL_RankLevel];
