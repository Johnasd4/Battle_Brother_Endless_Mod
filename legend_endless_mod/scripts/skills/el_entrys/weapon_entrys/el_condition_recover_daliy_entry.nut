this.el_condition_recover_daliy_entry <- this.inherit("scripts/skills/el_entrys/el_entry", {
	...
	function getTooltip( _id )
	{
		...
		local result = {
			id = _id,
			type = "text",
			text = "[color=" + colour + "]Recover " + this.m.EL_ConditionRecoverDaliy + "% Weapon Durability every day[/color]"
		};
		return result;
	}
	...
	function EL_getEntryColour()
	{
        ...
        {
            if (this.m.EL_ConditionRecoverDaliy <= this.Const.EL_Weapon.EL_Entry.Factor.EL_ConditionRecoverDaliy.ColourRange[index])
            {
	...
	function EL_createAddition()
	{
		...
		this.m.EL_ConditionRecoverDaliy = this.Const.EL_Weapon.EL_Entry.Factor.EL_ConditionRecoverDaliy.BaseConditionRecoverDaliy + this.Math.rand(randomMin, randomMax) * 0.01;
	}

	function EL_strengthen()
	{
		this.m.EL_ConditionRecoverDaliy = this.Const.EL_Weapon.EL_Entry.EntryStrengthenMult * this.Const.EL_Weapon.EL_Entry.Factor.EL_ConditionRecoverDaliy.ColourRange[this.Const.EL_Item.Type.Legendary];
	}
	...
	function EL_onUpgradeRank()
	{
		...
		{
			this.m.EL_ConditionRecoverDaliy += this.Const.EL_Weapon.EL_Entry.Factor.EL_ConditionRecoverDaliy.RandomMaxConditionRecoverDaliy[this.Const.EL_Item.Type.Normal] / 2 * 0.01;
		}
	}
	...
	function EL_refreshTotalEntry( _EL_totalEntry )
	{
		...
		_EL_totalEntry.m.EL_WeaponConditionRecoverDaliy += this.m.EL_ConditionRecoverDaliy;
	}
