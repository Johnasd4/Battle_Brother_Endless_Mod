this.el_condition_recover_rate_entry <- this.inherit("scripts/skills/el_entrys/el_entry", {
	...
	function EL_getEntryColour()
	{
        ...
        {
            if (this.m.EL_ConditionRecoverRate <= this.Const.EL_Weapon.EL_Entry.Factor.EL_ConditionRecoverRate.ColourRange[index])
            {
	...
	function EL_createAddition()
	{
		...
		this.m.EL_ConditionRecoverRate = this.Const.EL_Weapon.EL_Entry.Factor.EL_ConditionRecoverRate.BaseConditionRecoverRate + this.Math.rand(randomMin, randomMax) * 0.01;
	}

	function EL_strengthen()
	{
		this.m.EL_ConditionRecoverRate = this.Const.EL_Weapon.EL_Entry.EntryStrengthenMult * this.Const.EL_Weapon.EL_Entry.Factor.EL_ConditionRecoverRate.ColourRange[this.Const.EL_Item.Type.Legendary];
	}
	...
	function EL_onUpgradeRank()
	{
		...
		{
			this.m.EL_ConditionRecoverRate += this.Const.EL_Weapon.EL_Entry.Factor.EL_ConditionRecoverRate.RandomMaxConditionRecoverRate[this.Const.EL_Item.Type.Normal] / 2 * 0.01;
		}
	}
    ...
    function onTurnStart()
	{
		...
		if(weapon != null)
		{
			local condition_recover = this.Math.round(weapon.getConditionMax() * this.m.EL_ConditionRecoverRate * 0.01);
	...
	function EL_refreshTotalEntry( _EL_totalEntry )
	{
		...
		_EL_totalEntry.m.EL_WeaponConditionRecoverRate += this.m.EL_ConditionRecoverRate;
	}
