this.el_use_skill_fatigue_entry <- this.inherit("scripts/skills/el_entrys/el_weapon_entry", {
	m = {
	...
	function getTooltip( _id )
	{
		...
		local result = {
			id = _id,
			type = "text",
			text = "[color=" + colour + "]Weapon skills build up " + this.m.EL_UseSkillfatigueAddition + " less fatigue.[/color]"
		};
		return result;
	}
	...
	function EL_getEntryColour()
	{
        ...
        {
            if (this.m.EL_UseSkillfatigueAddition <= this.Const.EL_Weapon.EL_Entry.Factor.EL_UseSkillfatigue.ColourRange[index])
            {
	...
	function EL_createAddition()
	{
		...
		this.m.EL_UseSkillfatigueAddition = this.Const.EL_Weapon.EL_Entry.Factor.EL_UseSkillfatigue.BaseUseSkillfatigue + this.Math.rand(randomMin, randomMax);
	}
