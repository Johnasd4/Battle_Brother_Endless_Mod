this.el_hitpoints_entry <- this.inherit("scripts/skills/el_entrys/el_accessory_entry", {
	...
	function create()
	{
		...
		this.m.ID = this.Const.EL_Accessory.EL_Entry.Factor.EL_Hitpoints.ID;
	}
	...
	function getTooltip( _id )
	{
		...
		if(this.m.EL_CurrentLevel != 1)
		{
			return {
				id = _id,
				type = "text",
				text = "[color=" + colour + "]生命值 + " + this.Math.round(this.m.EL_CurrentLevel * this.m.EL_Hitpoints * 100) * 0.01 + "% (" + this.m.EL_Hitpoints + "%)[/color]"
			};
		}
		else
		{
