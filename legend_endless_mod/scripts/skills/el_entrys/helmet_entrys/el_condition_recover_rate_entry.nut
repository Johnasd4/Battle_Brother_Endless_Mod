this.el_condition_recover_rate_entry <- this.inherit("scripts/skills/el_entrys/el_accessory_entry", {
    ...
    function onTurnStart()
	{
		...
		if(helmet != null)
		{
			local condition_recover = this.Math.round(helmet.getConditionMax() * this.m.EL_CurrentLevel * this.m.EL_ConditionRecoverRate * 0.01);
	...
	function EL_refreshTotalEntry( _EL_totalEntry )
	{
		...
		_EL_totalEntry.m.EL_HelmetConditionRecoverRate += this.Math.round(this.m.EL_CurrentLevel * this.m.EL_ConditionRecoverRate * 100) * 0.01;
	}
