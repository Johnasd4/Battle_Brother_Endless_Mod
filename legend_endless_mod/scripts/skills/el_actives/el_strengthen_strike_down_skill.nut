this.el_strengthen_strike_down_skill <- this.inherit("scripts/skills/skill", {
	m = {
		StunChance = 75
	},
	function create()
	...
	function getTooltip()
	{
		...
		if (this.getContainer().getActor().getCurrentProperties().IsSpecializedInMaces)
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/special.png",
				text = "有[color=" + this.Const.UI.Color.PositiveValue + "]100%[/color]几率在一次打击中击昏三个回合"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/special.png",
				text = "有[color=" + this.Const.UI.Color.PositiveValue + "]" + this.m.StunChance + "%[/color]几率在一次打击中击昏三个回合"
			});
		}
	...
	function onUse( _user, _targetTile )
	{
		...
		if (success && target.isAlive())
		{
			if ((_user.getCurrentProperties().IsSpecializedInMaces || this.Math.rand(1, 100) <= this.m.StunChance) && !target.getCurrentProperties().IsImmuneToStun && !target.getSkills().hasSkill("effects.stunned"))
			{
				...
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " has stunned " + this.Const.UI.getColorizedEntityName(target) + " for three turns");
				}
			}
		}
