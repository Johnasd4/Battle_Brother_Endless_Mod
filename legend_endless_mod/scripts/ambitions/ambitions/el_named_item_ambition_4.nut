this.el_named_item_ambition_4 <- this.inherit("scripts/ambitions/ambition", {
	m = {
	...
	function create()
	{
		...
		this.m.ID = "ambition.el_named_item_4";
	...
	{
		if (!this.World.Ambitions.getAmbition("ambition.el_named_item_4").isDone())
		{
			return;
		}
	...
	function onReward()
	{
		...
		local item = this.new("scripts/items/el_accessory/el_named_item_ambition_item");
		stash.add(item);
		this.m.SuccessList.push({
			id = 10,
